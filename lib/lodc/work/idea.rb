...
module LODChallenge
  ...
  class Work
    ...
    class Idea < Work
      attr_accessor :title, :creators, :uri, :homepage, :lodc_entry_page, :description, :related_works, :year
      ...
      def extract_title(parser)
        parser.at_xpath("//tr[th/text()='アイデアの名称']/td").text.strip rescue nil
      end

      # 作品にアクセスするためのURIを取得する
      #   データセットやデモ用アプリケーション，アップロードしたアイデアのファイルなどにアクセスするためのURI
      def extract_homepage(parser)
        parser.at_xpath("//tr[th/text()='投稿したアイデア']/td/a/@href").text.strip rescue nil
      end

      # 作品の概要を取得する
      def extract_description(parser)
        parser.at_xpath("//tr[th/text()='アイデアの概略説明']/td").text.strip rescue nil
      end

      # 作品情報をRDF化する
      def to_rdf(format = :ntriples)
        graph = RDF::Graph.new
        graph << RDF::Statement.new(RDF::URI.new(@uri), RDF.type, RDF::LODC.Idea)
