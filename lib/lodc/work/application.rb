  ...
  class Work
    ...
    class Application < Work
      ...
      def extract_title(parser)
        parser.at_xpath("//tr[th/text()='アプリケーションの名称']/td").text.strip rescue nil
      end

      # 作品にアクセスするためのURIを取得する
      #   データセットやデモ用アプリケーション，アップロードしたアイデアのファイルなどにアクセスするためのURI
      def extract_homepage(parser)
        parser.at_xpath("//tr[th/text()='アプリケーションのURL']/td/a/@href").text.strip rescue nil
      end

      # 作品の概要を取得する
      def extract_description(parser)
        parser.at_xpath("//tr[th/text()='アプリケーションの概略説明']/td").text.strip rescue nil
      end

      # 作品の権利指定を取得する
      def extract_license(parser)
        license = parser.at_xpath("//tr[th/text()='アプリケーションの権利指定']/td//div[@class='title']").text.strip rescue nil
      ...
      def to_rdf(format = :ntriples)
        graph = RDF::Graph.new
        graph << RDF::Statement.new(RDF::URI.new(@uri), RDF.type, RDF::LODC.Application)
